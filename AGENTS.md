...
## Overview
Repo for an **autonomous overnight investigative research agent** that runs on a Mac, uses LM Studio for local model work and OpenRouter for cloud synthesis, and persists everything as markdown + SQLite. The Python package (`research_agent`, CLI `research`) is now scaffolded under `src/` and is being filled in issue-by-issue via alpha-loop. The top-level `*.md` playbooks remain the strategic source of truth and are themselves project deliverables. Output of this work feeds the broader `Alpha*` agent ecosystem.
...
- Sources (in-tree connectors): **Playwright**-driven web search and per-source recipes (`tools/browser.py`), `httpx` + `trafilatura` + `readability-lxml` (`web_fetch`), `arxiv` + `feedparser` (arXiv, news), `waybackpy` (archive), local PDFs/notes via `pypdf` + `unstructured` (`local_corpus`); GitHub via the operator's `gh` CLI
...
- `src/research_agent/`
  - `cli.py`, `daemon.py`, `intake.py`, `doctor.py`, `config.py` — top-level CLI surface and process entry points
  ...
  - `tools/` — `browser.py`, `web_search.py`, `web_fetch.py`, `news.py`, `reddit.py`, `arxiv_tool.py`, `archive.py`, `local_corpus.py`, `models.py`
  ...
  - `prompts/` — agent-persona templates as markdown (`planner.md`, `researcher.md`, `critic.md`, `synthesizer.md`, `intake_followup.md`); loaded via `prompts/loader.py`, packaged via `[tool.setuptools.package-data]`
...
- **Prompts live in `src/research_agent/prompts/*.md`**, not as inline string literals. Add new agent personas as a new markdown file and load via `prompts.loader`. Editing a persona's behavior means editing the markdown, not the Python.
...
- **No paid third-party data APIs.** All web search, news, and social scraping go through Playwright against public sites via the shared session manager in `tools/browser.py`. Free public APIs that don't require app registration are OK (arXiv, Wayback, GitHub via the operator's already-authenticated `gh` CLI). The only paid third party in v1 is **OpenRouter** for cloud LLM synthesis. **Do not add** Tavily, Brave Search, NewsCatcher, Reddit OAuth/PRAW, SerpAPI, or similar — add a Playwright recipe instead. (Note: `reddit.py` is a Playwright/public-endpoint connector, not PRAW — keep it that way.)
