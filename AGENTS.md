...
- **No paid third-party data APIs.** All web search, news, and social scraping go through Playwright against public sites via the shared session manager in `tools/browser.py` (built in issue #14). Free public APIs that don't require app registration are OK (arXiv, Wayback, GitHub via the operator's already-authenticated `gh` CLI). The only paid third party in v1 is **OpenRouter** for cloud LLM synthesis. **Do not add** Tavily, Brave Search, NewsCatcher, Reddit OAuth/PRAW, SerpAPI, or any similar service. If a connector seems to need one, add a Playwright recipe instead — that's the design.
