...
- CLI surface: `research start`, `list`, `status`, `view`, `search`, `stop`, `resume`, `logs`, `export`, `compare`, `doctor`, and `config cache-clear`
- Storage: **SQLite** in WAL mode at `data/index.sqlite` for jobs, tasks, plans, checkpoints, events, sources, findings, critiques, FTS5, embeddings, completion reasons, and LLM call ledger; separate `data/llm_cache.sqlite` for wipeable LLM cache; markdown + JSON sidecars for per-job content
...
- `src/research_agent/`
  ...
  - `orchestrator/` - planner/task loop, synthesis, critique, checkpointing, retry/error boundaries, scope-aware replanning, cornerstone extraction/querying, and translation pass
...
- **`web_fetch` is the URL router.** It handles robots checks, HTTP/readability extraction, Playwright fallback, PDF/audio/image shortcuts before host dispatch, connector-owned URLs, and best-effort archive spawning.
...
- **Emit structured observability.** Tool calls, model calls, decisions, warnings, checkpoints, skill loads, fan-outs, completion reasons, and cap diagnostics append to `events.jsonl` and mirror into SQLite via `observability/events.py`.
